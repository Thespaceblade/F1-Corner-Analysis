 ...
 * 
 * Usage: node scripts/validate_corner_coordinates.js
 */
 ...
 */
function loadSvgViewBox(svgFile) {
  ...
  try {
    ...
    console.error(`Error reading SVG file ${svgFile}:`, error.message)
    return null
  }
}
 ...
 */
function validateCornerCoordinates(corners, viewBox) {
  const errors = []
  const warnings = []
  let outOfBounds = 0
 ...
 */
function validateTrack(trackId, track) {
  const result = {
    track: trackId,
  ...
  const viewBox = loadSvgViewBox(track.svgFile)
...
function main() {
  ...
  for (const [trackId, track] of Object.entries(tracks)) {
    const result = validateTrack(trackId, track)
    results.push(result)
  }
